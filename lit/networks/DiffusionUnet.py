...
class DiffusionModelUNetVINN(nn.Module):
    """
    ...
    def __init__(
        ...
        self.use_fp16_VINN = use_fp16_VINN
        self.is_vinn = is_vinn

        assert(len(self.internal_size) == spatial_dims), "internal_size should have the same length as spatial_dims"
        ...
        self.conv_in = Convolution(
                ...
                padding=1,
                conv_only=True,
                #dtype=torch.float16 if self.use_fp16_VINN else torch.float32,
            )

        if self.use_fp16_VINN: self.conv_in = self.conv_in.to(torch.float16)


        # VINN layers
        if spatial_dims == 3: 
            self.interpol1 = il.Zoom3d(
                interpolation_mode=self.interpolation_mode,
                crop_position="front_top_left",
            ) 
            self.interpol2 = il.Zoom3d(
                interpolation_mode=self.interpolation_mode,
                crop_position="front_top_left",
            )
        else:
        ...
        for i in range(len(num_channels)):
            ...
            down_block = get_down_block(
                ...
                norm_num_groups=norm_num_groups,
                norm_eps=norm_eps,
                add_downsample=not (is_final_block or (is_first_block and self.is_vinn)),
                resblock_updown=resblock_updown,
        ...
        for i in range(len(reversed_block_out_channels)):
            ...
            is_final_block = i == len(num_channels) - 1
            is_second_to_last_block = i == len(num_channels) - 2
            ...
            up_block = get_up_block(
                ...
                norm_num_groups=norm_num_groups,
                norm_eps=norm_eps,
                add_upsample=not (is_final_block or (is_second_to_last_block and self.is_vinn)),
                resblock_updown=resblock_updown,
        ...
        self.out = nn.Sequential(
            nn.GroupNorm(num_groups=norm_num_groups, num_channels=num_channels[0], 
                         eps=norm_eps, affine=True),
            nn.SiLU(),
            zero_module(
                Convolution(
                    spatial_dims=spatial_dims,
        ...
        if self.use_fp16_VINN: self.out = self.out.to(torch.float16)
    ...
    def forward(
        ...
        t_emb = t_emb.to(dtype=x.dtype if not self.use_fp16_VINN else torch.float32)
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            ...
            #print(len(res_samples), [i.shape for i in res_samples])
            
            for residual in res_samples:
                down_block_res_samples.append(residual)
            
            if is_first_block and self.is_vinn: # instead of downsampling do VINN  
                assert(downsample_block.downsampler is None)
                assert(h.shape[3] > self.internal_size[0]), "internal_size should be smaller than input size // 2"
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            is_second_to_last = i == len(self.down_blocks) - 2
            
            res_samples = down_block_res_samples[-len(upsample_block.resnets) :]
            down_block_res_samples = down_block_res_samples[: -len(upsample_block.resnets)]
            h = upsample_block(hidden_states=h, res_hidden_states_list=res_samples, temb=emb, context=context)

            if is_second_to_last and self.is_vinn: # instead of downsampling do VINN  
                assert(upsample_block.upsampler is None)
                assert(h.shape[3] < output_shape[0]), "output_shape should be larger than internal size"
                h, _ = self.interpol1(h, scale_factors=1/scale_factors, target_shape=output_shape)
